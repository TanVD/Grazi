...
class GraziFUStateCollector : ApplicationUsagesCollector() {
    ...
    override fun getMetrics(): MutableSet<MetricEvent> {
        ...
        addIfDiffers(metrics, state, default, { s -> s.nativeLanguage.shortCode }, "native")
        addBoolIfDiffers(metrics, state, default, { s -> s.enabledSpellcheck }, "spellcheck")

        state.userEnabledRules.forEach {
            metrics.add(newMetric("rule", FeatureUsageData().addData("id", it).addData("enabled", true)))
        }

        state.userDisabledRules.forEach {
            metrics.add(newMetric("rule", FeatureUsageData().addData("id", it).addData("enabled", false)))
        }
